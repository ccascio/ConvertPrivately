...
import { validateJson } from "@convertprivately/core/data/json-validate";
...
export function JsonValidator({ hideAttribution, className, defaultText = "" }: JsonValidatorProps) {
  const [input, setInput] = useState(defaultText);
  const result = useMemo(() => (input.trim() ? validateJson(input) : null), [input]);

  return (
    <div className={`cp-tool cp-json-validator ${className ?? ""}`}>
